#!/usr/bin/env bash
# Run the calipers headless test suite against the Windows Blender
...
# NOTE: ../probes/*.py are one-shot API probes, not tests — they are
# deliberately not run here.
status=0
run_one "$TESTS_DIR/test_estimate.py" "CALIPERS_ESTIMATE_TESTS_PASSED" || status=1
run_one "$TESTS_DIR/test_core.py" "CALIPERS_CORE_TESTS_PASSED" || status=1
run_one "$TESTS_DIR/test_register.py" "CALIPERS_REGISTER_TESTS_PASSED" || status=1
run_one "$TESTS_DIR/test_overlay.py" "CALIPERS_OVERLAY_TESTS_PASSED" || status=1
