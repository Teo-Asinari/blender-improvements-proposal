...
"""Pure debounce logic for the live estimate refresh (same pattern as
uv_island_overlay/live.py).
...
(the poll in ``__init__``). This matters because app timers never fire
in ``blender --background`` (probed on 5.1.2), so anything that needs
the timer to be correct would be untestable headlessly.
"""
